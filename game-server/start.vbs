...
If Not fso.FolderExists(libsDir) Then
	...
	Else
		MsgBox "ERRO: Pasta libs nao encontrada:" & vbCrLf & libsDir, vbCritical, "Aion Emu - Game Server"
	End If
	WScript.Quit 1
End If

Do
	command = "cmd /c cd /d """ & serverDir & """ && title Aion Emu - Game Server && java -Xms1024m -Xmx2560m -XX:+UseNUMA -XX:+UseCompactObjectHeaders -DconsoleEncoding=CP850 -cp "".;config;" & libsDir & "\*"" com.aionemu.gameserver.GameServer 2>&1"
	If dashboardMode Then
...
If exitCode <> 0 Then
	If dashboardMode Then
		WScript.StdErr.WriteLine "Game server terminou com erro. Codigo: " & exitCode
	Else
		MsgBox "Game server terminou com erro. Codigo: " & exitCode, vbExclamation, "Aion Emu - Game Server"
	End If
End If
